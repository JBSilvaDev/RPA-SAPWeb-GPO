...
class ControllerLogin:
  ...
  def _find_all(self, firebase_folder:str):
      request = requests.get(f'{self.link_firebase}/{firebase_folder}/.json', verify=False)
      user = request.json()
      return user
  ...
  def last_run(self, map:dict):
      map = json.dumps(map)
      print(map)
      request = requests.post(f'{self.link_firebase}/Lasts_runs/.json', data=map, verify=False)
      print(request)
